# Client extras — `lk-flood-api-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `utilities`.

- Python: `python/lk_flood_api_docs/models.py`
...
```python
from lk_flood_api_docs import LkFloodApiDocsClient, iter_lab_endpoint

with LkFloodApiDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { LkFloodApiDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new LkFloodApiDocsClient();
...
```python
from lk_flood_api_docs import shard_groups, shard_page_numbers, shard_offsets
